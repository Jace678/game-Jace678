...
outcomes = {
    #('X', 'win'), ('O', 'win'),   # depth: {(player, outcome)} unique tuples
}
...
for batch in mod.games:
    ...
    try:
        ...
        print('games["%s"]["evaluation"] is missing or defective.' % (batch))
        if not skipErrors:
            sys.exit(1)
    if evaluation == 'play':
        if not playGames:
    ...
    try:
        ...
        print('games["%s"]["instances"] are missing or defective.' % batch)
        if not skipErrors:
            sys.exit(1)
    try:
        ...
        # print('games["%s"]["methods"] are missing or defective.' % batch)
        print('games["%s"]["players"] are missing or defective.' % batch)
        if not skipErrors:
            sys.exit(1)
...
for batch in gradeInfo:
    ...
    for pLabel in gradeInfo[batch]:
        ...
        for label in allScores:
            allScores[label] = max(allScores[label], maxScores[batch][pLabel][label])
